...
def get_TissueWiseSeg(prediction_matrix, gt_matrix, tissue_type):
    ...
    if tissue_type == 'WT':
        ...
        prediction_matrix = prediction_matrix[2]
        gt_matrix = gt_matrix[2]
    # print((prediction_matrix==1).sum(), (gt_matrix==1).sum())
...
def get_LesionWiseScores(prediction_seg, gt_seg, label_value, dil_factor):
    ...
    if isinstance(prediction_seg, str):
        pred_mat = np.load(prediction_seg)
    else:
        pred_mat = prediction_seg.copy()
    
    if isinstance(gt_seg, str):
        gt_mat = np.load(gt_seg)
    else:
        gt_mat = gt_seg.copy()
    ...
    fp = []
    metric_pairs = []
    # print(np.max(gt_label_cc), np.max(pred_label_cc))
    # print([np.sum(pred_label_cc==i+1) for i in range(np.max(pred_label_cc))])
    # print('/'*20)
...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    for l in range(len(label_values)):
        ...
        aux_metrics_dict[label_values[l]] = metric_df_thresh
    ...
    if output:
        results_df.to_csv(f"{output}.csv", index=False)
        for key, val in aux_metrics_dict.items():
            val.to_csv(f"{output}|{key}.csv", index=False)
